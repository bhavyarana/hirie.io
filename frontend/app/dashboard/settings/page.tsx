...
export default function SettingsPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '560px' }}>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.25rem' }}>Settings</h1>
      <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', fontSize: '0.875rem' }}>Manage your account preferences</p>

      {/* Profile info */}
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
        <h2 style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: '1rem', fontSize: '1rem' }}>Account</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Name</span>
            <span style={{ color: 'var(--text-primary)', fontSize: '0.875rem' }}>{user?.name || '—'}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Email</span>
            <span style={{ color: 'var(--text-primary)', fontSize: '0.875rem' }}>{user?.email}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem' }}>
        <h2 style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: '0.25rem', fontSize: '1rem' }}>Change Password</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '1.25rem' }}>You must enter your current password to confirm changes.</p>

        <form onSubmit={handleChangePassword} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label style={labelStyle}>Current Password</label>
            <input type="password" value={oldPassword} onChange={e => setOldPassword(e.target.value)} placeholder="Enter your current password" required style={inputStyle} />
          </div>
          <div>
            ...
            <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Min. 6 characters" required minLength={6}
              style={{ ...inputStyle, borderColor: passwordMismatch ? '#ef4444' : passwordsMatch ? '#22c55e' : 'var(--border)' }} />
          </div>
          <div>
            <label style={labelStyle}>Confirm New Password</label>
            <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Re-enter new password" required minLength={6}
              style={{ ...inputStyle, borderColor: passwordMismatch ? '#ef4444' : passwordsMatch ? '#22c55e' : 'var(--border)' }} />
          ...
          <button type="submit" disabled={loading || !!passwordMismatch || !oldPassword || !newPassword} style={{
