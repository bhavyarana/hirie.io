...
import { submissionsApi, SubmissionRecruiterDetail } from '@/lib/api';
...
export default function RecruiterDetailPage() {
  ...
  const { data, isLoading, error } = useQuery<SubmissionRecruiterDetail>({
    queryKey: ['submission-recruiter-detail', id, dateFrom, dateTo],
    queryFn: () => submissionsApi.recruiterDetail(id, params),
    enabled: !!id,
    staleTime: 30_000,
  });
  ...
  if (isLoading) return (
    <div style={{ padding: '2rem', color: 'var(--text-muted)', textAlign: 'center' }}>
      <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>⏳</div>Loading recruiter detail…
    </div>
  );
  ...
  const { recruiter, kpis, jobs, candidates, timeline } = data;

  // Jobs bar chart data
  const jobChartData = jobs.slice(0, 8).map(j => ({ name: j.title.length > 18 ? j.title.slice(0, 18) + '…' : j.title, submissions: j.submission_count }));
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1100px' }}>
      ...
      <div style={{ marginBottom: '1.75rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)' }}>{recruiter.name}</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>
            {recruiter.email}
            {recruiter.role && <span style={{ marginLeft: '0.5rem', padding: '0.1rem 0.45rem', borderRadius: '999px', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', background: 'rgba(99,102,241,0.1)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.2)' }}>{recruiter.role}</span>}
          </p>
          {(dateFrom || dateTo) && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '1rem', marginBottom: '1.75rem' }}>
        ...
        <KpiCard icon="💼" label="Jobs Worked On" value={kpis.jobs_count} color="#6366f1" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.75rem' }}>
          ...
          {timeline.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={timeline} margin={{ top: 4, right: 4, left: -28, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="count" stroke="#22c55e" fill="url(#green-grad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div style={card()}>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1rem' }}>💼 Passes by Job</p>
          {jobChartData.length === 0 ? (
            <p style={{ color: 'var(--text-faint)', fontSize: '0.8rem' }}>No data.</p>
          ) : (
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={jobChartData} layout="vertical" margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="submissions" fill="#6366f1" radius={[0, 3, 3, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div style={{ ...card(), marginBottom: '1.5rem' }}>
        <p style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: '1rem' }}>💼 Jobs Worked On</p>
        <DataTable
      ...
      <div style={card()}>
        ...
        <DataTable
          headers={['Candidate', 'Email', 'Job', 'Score', 'Submitted At']}
