...
function CreateTeamModal({ onClose, managers, tls, allUsers }: {
  onClose: () => void; managers: UserRecord[]; tls: UserRecord[]; allUsers: UserRecord[];
}) {
  ...
  const eligible = allUsers.filter(u => (u.role === 'tl' || u.role === 'recruiter') && !addedIds.has(u.id));
  ...
  function addMember() {
    ...
    setAddUserId(''); setAddRole('recruiter');
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await Promise.all(members.map(m =>
        teamsApi.addMember(teamId, { user_id: m.userId, role_in_team: m.roleInTeam }).catch(() => null)
      ));
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '2rem', width: '480px', maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, marginBottom: '1.5rem' }}>Create Team</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          <div>
            ...
            {role === 'admin' ? (
              ...
              <div style={{ ...INP, color: '#a5b4fc', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.25)', cursor: 'not-allowed' }}>
                {user?.name || user?.email || '—'} <span style={{ color: 'var(--text-faint)', fontSize: '0.75rem' }}>(you)</span>
              </div>
            )}
          </div>
          ...
          <div>
            <label style={LBL}>Add Members <span style={{ color: 'var(--text-faint)' }}>(optional)</span></label>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
              <select value={addUserId} onChange={e => setAddUserId(e.target.value)} style={{ ...INP, flex: 1 }}>
                <option value="">— Select user —</option>
                {eligible.map(u => <option key={u.id} value={u.id}>{u.name || u.email} ({u.role})</option>)}
              </select>
              <select value={addRole} onChange={e => setAddRole(e.target.value as 'tl' | 'recruiter')} style={{ ...INP, width: '110px', flexShrink: 0 }}>
                <option value="recruiter">Recruiter</option>
                <option value="tl">TL</option>
              </select>
              <button type="button" onClick={addMember} disabled={!addUserId}
                style={{ padding: '0 0.875rem', borderRadius: '0.5rem', background: addUserId ? '#6366f1' : 'var(--border)', color: addUserId ? '#fff' : 'var(--text-faint)', border: 'none', cursor: addUserId ? 'pointer' : 'not-allowed', fontSize: '1rem', flexShrink: 0 }}>
                +
              </button>
            </div>
            ...
            {members.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem', marginTop: '0.25rem' }}>
                {members.map(m => (
                  <span key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.625rem', borderRadius: '999px', fontSize: '0.75rem', background: m.roleInTeam === 'tl' ? 'rgba(34,197,94,0.1)' : 'rgba(56,189,248,0.1)', color: m.roleInTeam === 'tl' ? '#22c55e' : '#38bdf8', border: `1px solid ${m.roleInTeam === 'tl' ? 'rgba(34,197,94,0.3)' : 'rgba(56,189,248,0.3)'}` }}>
                    {m.label}
                    <span style={{ opacity: 0.6, fontSize: '0.65rem' }}>({m.roleInTeam === 'tl' ? 'TL' : 'Rec.'})</span>
                    <button type="button" onClick={() => removeMember(m.userId)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'currentColor', padding: 0, lineHeight: 1, fontSize: '0.9rem', opacity: 0.7 }}>×</button>
                  </span>
                ))}
              </div>
            )}
            {members.length === 0 && (
              <p style={{ color: 'var(--text-faint)', fontSize: '0.75rem', marginTop: '0.25rem' }}>No members added yet — you can add them after creation too.</p>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)' }}>Teams</h1>
      ...
      {/* Grid */}
      {isLoading ? (
        <div style={{ color: 'var(--text-muted)', padding: '2rem', textAlign: 'center' }}>Loading teams...</div>
      ) : teams.length === 0 ? (
        <div style={{ padding: '4rem', textAlign: 'center', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem' }}>
          ...
          {canCreate && (
            <button onClick={() => setShowCreate(true)} style={{ marginTop: '1rem', padding: '0.625rem 1.25rem', borderRadius: '0.5rem', background: '#6366f1', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '0.875rem' }}>
              + Create First Team
            </button>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
          {teams.map((team: Team) => (
            <div key={team.id}
              style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem', transition: 'border-color 0.2s' }}
              onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(99,102,241,0.4)'}
              onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--border)'}>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div>
                  <h3 style={{ color: 'var(--text-primary)', fontWeight: 600, marginBottom: '0.375rem' }}>{team.name}</h3>
                ...
                {role === 'admin' && (
                  <button onClick={() => { if (confirm('Delete this team?')) deleteMutation.mutate(team.id); }}
                    style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '0.25rem', fontSize: '0.875rem', borderRadius: '0.25rem' }}
