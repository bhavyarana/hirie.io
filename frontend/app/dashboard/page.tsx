...
function StatCard({ label, value, icon, color, isLoading }: { label: string; value: number | string; icon: string; color: string; isLoading?: boolean }) {
  return (
    <div style={{
      background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.25rem',
  ...
  return (
    <>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Full platform overview</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        ...
        <Link href="/dashboard/jobs/new" style={{ padding: '0.75rem 1.5rem', borderRadius: '0.625rem', border: '1px solid var(--border)', color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.875rem' }}>+ Create Job</Link>
      </div>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', overflow: 'hidden', marginBottom: '1.5rem' }}>
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '1rem' }}>Teams</h2>
            ...
            <tbody>
              {teams.slice(0, 5).map(team => (
                <tr key={team.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '1rem 1.5rem', color: 'var(--text-primary)', fontWeight: 500 }}>{team.name}</td>
                  <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)', fontSize: '0.875rem' }}>{team.manager?.name || team.manager?.email || '—'}</td>
                  <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)', fontSize: '0.875rem' }}>{team.tl?.name || team.tl?.email || '—'}</td>
                  <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>{team.member_count ?? 0}</td>
...
function ManagerDashboard() {
  ...
  return (
    <>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)' }}>Manager Dashboard</h1>
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Your teams and jobs at a glance</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        ...
        <Link href="/dashboard/teams" style={{ padding: '0.75rem 1.5rem', borderRadius: '0.625rem', border: '1px solid var(--border)', color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.875rem' }}>🏢 Manage Teams</Link>
      </div>
...
function TLDashboard() {
  ...
  return (
    <>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)' }}>Team Lead Dashboard</h1>
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Monitor your team's candidate pipeline</p>
      </div>
  ...
  return (
    <>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Your assigned jobs and candidates</p>
      </div>
...
function RecentJobsTable({ isLoading, jobs, showCreateBtn = true }: { isLoading: boolean; jobs: any[]; showCreateBtn?: boolean }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '1rem', overflow: 'hidden' }}>
      <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '1rem' }}>Recent Jobs</h2>
      ...
      {isLoading ? (
        ...
        <div style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>No jobs yet</p>
          {showCreateBtn && (
          ...
          <tbody>
            {jobs.slice(0, 5).map(job => (
              <tr key={job.id} style={{ borderTop: '1px solid var(--border)', transition: 'background 0.15s' }}
                ...
                <td style={{ padding: '1rem 1.5rem', color: 'var(--text-primary)', fontWeight: 500 }}>{job.job_title}</td>
                <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>{job.company_name}</td>
                <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{job.team?.name || '—'}</td>
                <td style={{ padding: '1rem 1.5rem' }}>
                  <span style={{
                ...
                <td style={{ padding: '1rem 1.5rem', color: 'var(--text-secondary)' }}>{job.candidate_count || 0}</td>
                <td style={{ padding: '1rem 1.5rem', color: 'var(--text-muted)', fontSize: '0.8rem' }}>{new Date(job.created_at).toLocaleDateString()}</td>
                <td style={{ padding: '1rem 1.5rem' }}>
